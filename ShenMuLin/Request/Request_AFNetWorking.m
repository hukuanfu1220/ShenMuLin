...
@interface Request_AFNetWorking()
@property (nonatomic, strong) AFHTTPSessionManager *sessionManager;
@end
@implementation Request_AFNetWorking
static  Request_AFNetWorking *networkManager = nil;
/**
 *  单例
 *
...
+ (instancetype)defaultManager {
    ...
    dispatch_once(&onceToken, ^{
        if (!networkManager) {
            networkManager = [[Request_AFNetWorking alloc] init];
        }
    });
...
- (nullable NSURLSessionDataTask *)sendPOSTRequestWithserverUrl:(nonnull NSString *)serverUrl
    ...
    task = [self.sessionManager POST:requestPath parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (UIImage * image in imageArray) {
            ...
            // 拼接Data
            [formData appendPartWithFileData:imgData name:@"upfile" fileName:@"upfile.jpg" mimeType:@" image/jpeg"];
            i++;
        }
...
- (NSString *)failHandleWithErrorResponse:( NSError * _Nullable )error task:( NSURLSessionDataTask * _Nullable )task {
    ...
    DLog(@"afNetworking_errorMsg == %@",[[NSString alloc]initWithData:afNetworking_errorMsg encoding:NSUTF8StringEncoding]);
